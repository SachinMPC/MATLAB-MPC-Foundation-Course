...
% function [Sx_mat, Su_mat, H_mat, F_mat, A_mat, F_vec, B_mat, WX_mat, WU_mat, Ix_mat, Iu_mat] =...
%                                     Gen_QP_Matrices_Tracking(phy, gam, Wx, Wu, n_pred, u_L, u_H, deluL, deluH)

function [mpc] = Gen_QP_Matrices_Tracking( mpc )

% Create local matrices from mpc structure 

phy = mpc.phy ; gam = mpc.gam ; Wx = mpc.Wx ;  Wu = mpc.Wu ;  n_pred= mpc.n_pred; 
u_L = mpc.u_L ; u_H = mpc.u_H ; deluL = mpc.deluL ; deluH = mpc.deluH ; 
