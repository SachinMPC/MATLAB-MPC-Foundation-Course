...
function [mpc] = Gen_QP_Matrices_OP_Tracking_delU(mpc)

phy = mpc.phy ; gam = mpc.gam ; L_mat = mpc.L_mat ; C_mat =  mpc.C_mat ; 
Wy = mpc.Wy ;  Wu = mpc.Wu ;  Wdelu = mpc.Wdelu ;
...
if (spectral_radius < 1)
    Wx_inf = dlyap(phy',C_mat'*Wy*C_mat ) ;
end
Wy_inf = C_mat *  Wx_inf * C_mat' ; 
...
for k = 1 : n_pred
    ...
    if (k == n_pred )
        if (spectral_radius < 1)
            WY_mat = blkdiag(WY_mat, Wy_inf)  ;
        else
            WY_mat = blkdiag(WY_mat, Wy)  ;
        end
    else
        WY_mat = blkdiag(WY_mat, Wy)  ;
    end
    ...
    Se_mat(r1+1:r2,:) = C_mat * Se_row_mat + eye(n_op);    
end
...
H_mat = 2 * (Su_mat'* WY_mat * Su_mat + WU_mat  +  psi_mat' * WdelU_mat *  psi_mat) ;
H_mat=(H_mat+H_mat')/2;  % To avoid warnings in quadprog 
F_mat = 2 * Su_mat' * WY_mat * Sx_mat ; 
...
B_mat = zeros( 2*n_pred*n_ip, n_ip) ;
 ...
 mpc.H_mat = H_mat;  
 mpc. F_mat = F_mat ;  mpc.A_mat= A_mat ;  
 mpc.F_vec = F_vec ; mpc. B_mat = B_mat;  
 mpc.WY_mat = WY_mat ;  mpc.WU_mat = WU_mat ; mpc. WdelU_mat =  WdelU_mat ;
 mpc.Ix_mat = Ix_mat ;  mpc.Iu_mat = Iu_mat ; mpc.Ir_mat = Ir_mat ; 
